...
import { Card, Skeleton } from '@/workspace-ui/src';
...
async function UpcomingEvents() {
  ...
  if (!events.length) return <LumaCalendar />;
  return <ul className="public-event-list" role="list">{events.map(event => <li key={event.id}><time dateTime={event.startsAt}>{new Date(event.startsAt).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric', timeZone: 'Europe/London' })}<span>{new Date(event.startsAt).toLocaleTimeString('en-GB', { hour: '2-digit', minute: '2-digit', timeZone: 'Europe/London', timeZoneName: 'short' })}</span></time><div><h3>{event.title}</h3><p>{event.description}</p><p className="public-event-location">{event.venue || 'Location to be announced'}</p></div>{event.registrationUrl ? <ActionLink href={event.registrationUrl} external>Event details</ActionLink> : <span className="public-footnote">Registration opening soon</span>}</li>)}</ul>;
}
...
export default function EventsPage() {
  return <PublicShell>
    ...
    <section className="public-section" id="upcoming"><SectionHeading label="The calendar" title="See you at the next one." /><Suspense fallback={<Skeleton label="Loading upcoming events" />}><UpcomingEvents /></Suspense><div className="public-section-end"><a className="public-calendar-link" href={LUMA_URL} target="_blank" rel="noopener noreferrer">Full calendar on Luma <span aria-hidden="true">↗</span></a><p id="luma-privacy-note">Luma receives connection information and may use its own browser storage. Its registration and privacy settings apply.</p></div></section>
