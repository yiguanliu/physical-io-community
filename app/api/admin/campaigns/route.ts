...
export async function POST(req:Request){try{if(req.headers.get('origin')!==new URL(req.url).origin)return json({error:'Origin not allowed.'},403);const user=await authorize();if(!user)return json({error:'Admin access required.'},401);const raw=await req.text();if(raw.length>5000)return json({error:'Request too large.'},413);const input=deliveryCommand.parse(JSON.parse(raw));const db=await emailDatabase().connect();try{const result=await deliverBatch(db,input,{id:user.id,name:user.email??'Admin'},undefined,undefined,{enqueueOnly:input.action==='send'});if(input.action==='send')after(async()=>{await runCampaignWorker();});return json(result);}finally{db.release();}}catch(e){return failure(e);}}
