...
const paragraph = (text: string) => `<p style="margin:0 0 24px;font-size:20px;line-height:1.5;color:${emailTheme.ink};">${text}</p>`;
...
const security = paragraph('If you did not make this change, secure your account and <a href="https://www.physical-io.com/askusanything" style="color:#ffffff;text-decoration:underline;">contact us</a>.');

export const authEmailTemplates = [
 {name:'confirmation',subject:'Confirm your Physical I/O account',title:'Welcome to Physical I/O.',content:paragraph('Confirm your email address to finish creating your account.')+link('Confirm my email')+warning},
...
].map(template => ({...template, html:renderEmailHtml({previewText:template.subject,body:'',bodyHtml:`<h1 style="margin:0 0 24px;font-size:26px;font-weight:400;line-height:1.3;color:#ffffff;">${template.title}</h1>${template.content}`})}));
