...
export async function readWorkspace():Promise<WorkspaceData>{
 ...
 db.query(`select m.id,m.email,m.full_name,m.professional_role,m.city,m.status,m.signed_up_at,m.notes,m.website_url,m.linkedin_url,m.email_status,auth_account.created_at account_created_at,auth_account.last_sign_in_at,auth_account.email_confirmed_at,coalesce((select json_agg(json_build_object('topic',s.topic,'status',s.status)) from public.subscriptions s where s.member_id=m.id and s.channel='email'),'[]') subscriptions,
 ...
 from public.members m left join lateral (select u.created_at,u.last_sign_in_at,u.email_confirmed_at from auth.users u where lower(trim(u.email))=m.email_normalized and u.deleted_at is null and not coalesce(u.is_anonymous,false) order by u.created_at desc limit 1) auth_account on true order by m.created_at desc limit 1001`),
 ...
 db.query(`select c.*,coalesce((select json_object_agg(status,n) from (select status,count(*)::int n from public.campaign_recipients where campaign_id=c.id group by status) counts),'{}') delivery_counts from public.campaigns c order by updated_at desc limit 1001`),
 ...
 members:members.rows.slice(0,1000).map(m=>({id:m.id,name:m.full_name,email:m.email,role:m.professional_role,city:m.city,joined:new Date(m.signed_up_at).toLocaleDateString('en-GB'),status:title(m.status),topics:m.topics,notes:m.notes,website:m.website_url,linkedin:m.linkedin_url,emailStatus:m.email_status,accountCreatedAt:m.account_created_at?new Date(m.account_created_at).toISOString():null,lastSignInAt:m.last_sign_in_at?new Date(m.last_sign_in_at).toISOString():null,emailConfirmedAt:m.email_confirmed_at?new Date(m.email_confirmed_at).toISOString():null,subscriptions:m.subscriptions})),
 ...
 campaigns:campaigns.rows.slice(0,1000).map(c=>({id:c.id,name:c.name,type:title(c.type),status:title(c.status),body:c.body,audience:c.audience_filter?.leadId??(c.audience_filter?.requireConsent===false&&!c.audience_filter?.memberIds?'Email list':Object.keys(c.audience_filter??{}).length&&c.audience_filter?.kind!=='subscribed'?'Saved audience':'All opted-in'),date:new Date(c.updated_at).toLocaleString('en-GB'),delivery:c.status==='draft'?'Not sent':`${['sent','delivered','opened','clicked'].reduce((n,k)=>n+(c.delivery_counts[k]??0),0)} accepted · ${(c.delivery_counts.pending??0)+(c.delivery_counts.processing??0)} remaining · ${c.delivery_counts.failed??0} failed · ${c.delivery_counts.uncertain??0} uncertain · ${c.delivery_counts.skipped??0} skipped`})),
...
export async function mutate(db:PoolClient,c:Command,actor:{id:string;name:string;requestId?:string}){
 ...
 if(c.action==='event.feature'||c.action==='event.work.create'||c.action==='event.initiative.save'||c.action==='event.advance'||c.action==='event.archive'||c.action==='event.delete'){entityId=(await mutateEvent(db,c,actor)).id;
  ...
  if(!['Selected members','All opted-in','Email list','Saved audience'].includes(c.audience)&&!(await db.query('select id from public.leads where id=$1',[c.audience])).rowCount)throw new NotFound();
  const audience=c.audience==='Selected members'?{memberIds:c.memberIds,requireConsent:false}:c.audience==='Saved audience'?null:['All opted-in','Email list'].includes(c.audience)?{requireConsent:false}:{kind:'lead',leadId:c.audience};
