...
import { digest,emailConfiguration,deliverySettings,sendEmail,DeliveryError,type EmailTransport,type EmailPayload } from '@/lib/email/transport';
...
function filterFor(c:Record<string,any>){let raw=c.audience_filter;if(typeof raw==='string'){try{raw=JSON.parse(raw);}catch{throw new RecordConflict('Invalid saved audience. Edit the draft before sending.');}}const parsed=audienceSchema.safeParse(raw??{});if(!parsed.success)throw new RecordConflict('This audience is not supported for community batch email. Select members or use the email list.');return {...parsed.data,campaignType:c.type,requireConsent:false,statuses:parsed.data.statuses};}
export async function previewDelivery(db:PoolClient,id:string){const c=await campaign(db,id);const {eligible,skipped}=resolveAudience(await people(db),filterFor(c));const counts=(await db.query('select status,count(*)::int count from public.campaign_recipients where campaign_id=$1 group by status',[id])).rows;const failures=(await db.query("select id,member_id,name,email,status,skip_reason from public.campaign_recipients where campaign_id=$1 and status in ('failed','bounced','complained','uncertain') order by name,email,id",[id])).rows;return {failures,campaign:c,eligibleCount:eligible.length,skippedCount:skipped.length,sample:eligible.slice(0,8).map(m=>({name:m.fullName,email:m.email})),counts,confirmation:digest({id:c.id,version:c.updated_at,subject:c.subject,body:c.body,audience:c.audience_filter,recipients:eligible.map(m=>[m.id,m.email])}),...emailConfiguration()};}
...
export async function deliverBatch(db:PoolClient,input:z.infer<typeof deliveryCommand>,actor:Actor,transport:EmailTransport=sendEmail,config=deliverySettings(),options:{enqueueOnly?:boolean;deadline?:number}={}){
 ...
 try{
  ...
  if(c.status==='draft'){
   const preview=await previewDelivery(db,c.id);if(preview.confirmation!==input.confirmation)throw new RecordConflict('The draft or audience changed. Review the preview again.');if(!preview.eligibleCount)throw new RecordConflict('No eligible addresses match this audience.');
   ...
   await db.query('BEGIN');try{const locked=await db.query('select status,updated_at from public.campaigns where id=$1 for update',[c.id]);if(locked.rows[0].status!=='draft'||new Date(locked.rows[0].updated_at).getTime()!==new Date(c.updated_at).getTime())throw new RecordConflict('Draft changed. Preview again.');for(const m of eligible)await db.query('insert into public.campaign_recipients(campaign_id,member_id,email,name) values($1,$2,$3,$4)',[c.id,m.id,m.email,m.fullName]);for(const {member:m,reason} of skipped)await db.query("insert into public.campaign_recipients(campaign_id,member_id,email,name,status,skip_reason) values($1,$2,$3,$4,'skipped',$5)",[c.id,m.id,m.email,m.fullName,reason]);await db.query("update public.campaigns set status='sending',recipient_count=$2,skip_count=$3,updated_at=now() where id=$1",[c.id,eligible.length,skipped.length]);if(options.enqueueOnly)await log(db,c.id,'queued',{queued:true},actor,input.requestId);await db.query('COMMIT');}catch(e){await db.query('ROLLBACK');throw e;}
  }
  if(options.enqueueOnly){if(c.status==='draft')return {queued:true};const summary={queued:true};await log(db,c.id,'queued',summary,actor,input.requestId);return summary;}
  ...
  for(const r of pending){if(options.deadline&&Date.now()+70000>options.deadline)break;const m=(await people(db,r.member_id))[0];const reason=!m?'member_removed':m.email!==r.email?'email_changed':skipReasonForMember(m,filter);if(reason){await db.query("update public.campaign_recipients set status='skipped',skip_reason=$2 where id=$1",[r.id,reason]);continue;}
