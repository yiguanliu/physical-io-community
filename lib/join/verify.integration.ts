...
async function verify(){
 ...
 const input=joinSchema.parse({firstName:'Onboarding',lastName:'Test',email:'join-test-'+Date.now()+'@example.invalid',city:'London',role:'Engineer / Developer',experience:'Less than a year',work:'Test robotics',website:'example.com',linkedin:'https://www.linkedin.com/in/test-visitor',goals:['Learning from talks & demos'],formats:['Talks / panels'],suggestions:'Verification only',consent:true,updates:false});
 ...
 assert.equal(await saveSignup(db as unknown as PoolClient,{...input,firstName:'Overwrite',lastName:'attempt'},'duplicate-test'),undefined);
