...
export default function RobotCall(props:Props){
 ...
 const callControls=<>
  {state==='idle'?<Popover title="Call Ohi" closeLabel="Close call options" side={props.actionDock?'right':'bottom'} align="start" trigger={<IconButton label="Call Ohi" title="Call Ohi" variant="ghost"><Phone size={18}/></IconButton>}><div className={styles.callChoices}><Button variant="ghost" onClick={()=>void start('voice')}><Phone size={18}/>Voice call</Button><Button variant="ghost" onClick={()=>void start('video')}><Video size={18}/>Video call</Button></div></Popover>:<><span role="status">{state==='connecting'?'Connecting…':state==='video'?'Video call':'Voice call'}</span><IconButton label="End call" variant="primary" onClick={end}><PhoneOff size={18}/></IconButton></>}
