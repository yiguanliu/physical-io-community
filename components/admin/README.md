...
The Visuals tab now uses `MarketingVisual`, `MarketingCanvas`, `MarketingInspector` and the validated `lib/marketing/visual-model.ts` document model. `MarketingCalendar` supplies month/week scheduling and category badges. All new controls use the workspace library.

Apply `supabase/migrations/20260908204812_content_editor.sql` before releasing this version. It adds `content_categories` (RLS enabled, no anonymous/authenticated direct access), `content_items.category_id`, `featured`, and `visual_document`. The host's existing authorized transactional API owns all mutations, conflict checks and audit records. Existing stories receive Editorial; their topics and workflow remain unchanged. Presets reuse `content_templates.layout.document`.

Verification: `node_modules/.bin/vitest run lib/marketing/visual-model.test.ts`; `node --import tsx scripts/checks/content-editor-rollback.ts` validates the migration and new commands in a rolled-back transaction against the pre-migration schema. The latter loads the configured database and never commits. Browser checks use isolated test fixtures; live image upload/publishing must retain their existing credentials and authorization.

Event initiatives use `EventWorkspace`, `lib/events/model.ts` and the existing authenticated workspace API. Apply migration `20260908211557_event_initiatives.sql` before deploying this UI. `community_events` retains public-facing status; the private `event_initiatives` table stores versioned lifecycle state, tasks, links, reminders and approval history. Proposals may have no date. Save actions use optimistic version checks and transactional audit entries. New linked work creates draft records only. Deleting an episode preserves linked resources. Reminder banners are in-app; ICS export creates a separate calendar copy, not server email delivery.

Verify with `vitest run lib/events/model.test.ts`, the normal build, and `tsx scripts/checks/event-initiative-rollback.ts` (uses the configured database and rolls all test data back; applies the migration inside that rollback transaction when absent).
