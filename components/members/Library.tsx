...
import { Alert, Badge, Button, Card } from '@/workspace-ui/src';
...
export default function Library({ episodes }: { episodes: Episode[] }) {
  ...
  return <div className="member-library">
    <div className="member-heading"><div><p className="public-eyebrow">PHYSICAL I/O · ON DEMAND</p><h1>Good ideas deserve a replay.</h1><p>Past gatherings. Fresh perspectives. Watch at your own pace.</p></div><Button variant="ghost" busy={busy} onClick={signOut}>Sign out</Button></div>
    ...
    <section aria-labelledby="episodes-title" className="member-stack"><div className="member-heading"><h2 id="episodes-title">The event collection</h2><span className="member-note">{episodes.length} {episodes.length === 1 ? 'episode' : 'episodes'}</span></div>
      <div className="member-grid">{episodes.map(episode => <Card key={episode.id} className="member-episode">
        ...
        <div className="member-stack"><Badge>{episode.label}</Badge><h3>{episode.title}</h3><p>{episode.description}</p><div className="member-actions"><Button variant="primary" onClick={event => { opener.current = event.currentTarget; setWatching({ episode, start: 0 }); }}><Play size={16} aria-hidden /> Watch episode</Button><Button variant="ghost" onClick={event => { opener.current = event.currentTarget; setWatching({ episode, start: episode.start }); }}>Play from 43:09</Button></div></div>
      </Card>)}</div>
