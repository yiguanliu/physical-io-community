...
export default function EmailCodeForm({ configured, joined = false }: { configured: boolean; joined?: boolean }) {
  ...
  const [hasCode, setHasCode] = useState(joined);
  useEffect(() => { if (joined) { try { setEmail(sessionStorage.getItem('member-join-email') || ''); } catch {} } }, [joined]);
  const ready = Boolean(sent.notice) || hasCode;
  return <div className="member-stack">
    {joined && <p role="status">Your profile is saved. Enter the code we sent to your email to confirm it and sign in.</p>}
    <form action={send} className="member-stack">
      <Field name="email" label="Email address" type="email" autoComplete="email" value={email} onChange={event => setEmail(event.target.value)} required maxLength={254} readOnly={Boolean(sent.notice)} disabled={!configured || sending || verifying} />
