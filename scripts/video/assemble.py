...
OUT=BASE/'visuals/video'; OUT.mkdir(exist_ok=True)
FF=imageio_ffmpeg.get_ffmpeg_exe()
rows=json.loads((BASE/'video/veo-shot-prompts.json').read_text())
...
for row,duration in zip(rows,durations):
 ...
 for p in (BASE/'video/runs').glob(f'shot-{n}-*.json'):
  ...
  if job.get('state')=='complete':matches.append((p,job))
 if not matches:continue
 p,job=matches[-1]
 target=OUT/f'shot-{n}.mp4'
 run(['-i',job['output'],'-t',str(duration),'-vf','scale=1280:720,setsar=1,fps=24,format=yuv420p','-c:v','libx264','-crf','18','-c:a','aac','-ar','48000','-ac','2','-movflags','+faststart',str(target)])
 ready.append(n)
if len(ready)==6:
...
page='''<!doctype html><html lang="en"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Love, Mind + Body — film preview</title><link rel="stylesheet" href="assets/workspace.css"><link rel="stylesheet" href="gallery.css?v=04"><style>main{max-width:1200px;padding:32px}h1{font:400 clamp(36px,6vw,70px)/1.1 Rokkitt,serif}video{width:100%;aspect-ratio:16/9;background:#050505;display:block} .clips{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:36px 24px}h2{margin:32px 0 16px}h3{font:400 25px Rokkitt,serif;margin:14px 0 8px}.muted{color:#aaa;line-height:1.6}a{text-underline-offset:4px}.master{margin:32px 0 48px}details{margin-top:12px}summary{cursor:pointer}@media(max-width:650px){.clips{grid-template-columns:1fr}main{padding:20px}}</style><body class="wui" data-mode="dark"><main><a href="index.html">← Storyboard & posters</a><p class="eyebrow" style="margin-top:36px">PHYSICAL I/O · EPISODE 02 · FIRST MOTION CUT</p><h1>Love, Mind + Body</h1><p class="muted">Six shots. Thirty seconds. Black-and-white physical interfaces.<br>Sound-led review cut · No voiceover · Press play to hear the generated sound.</p>'''
if (OUT/'love-mind-body-30s.mp4').exists():
 page+='''<section class="master"><h2>The 30-second film</h2><video controls playsinline preload="metadata" poster="assets/shot-01-v2.png" src="video/love-mind-body-30s.mp4" aria-label="Complete thirty-second film"></video><p><a href="video/love-mind-body-30s.mp4" download>Download the film</a></p></section>'''
...
for row,d in zip(rows,durations):
 n=row['shot'];page+=f'<article><p class="eyebrow">SHOT {n} · {row["edit_duration"]} · {d} SEC</p>'
 if n in ready:page+=f'<video controls playsinline preload="metadata" poster="assets/shot-{n}-v2.png" src="video/shot-{n}.mp4" aria-label="Shot {n}: {html.escape(row["title"])}"></video>'
 ...
 if n in ready:page+=f'<a href="video/shot-{n}.mp4" download>Download shot {n}</a>'
 if n=='03':page+='<p class="muted">Review note: Veo introduced a hand turning the control. This take departs from the intended hardware-only frame.</p>'
...
(BASE/'visuals/film.html').write_text(page)
